...
const CONTRACT_VALUE = 117059.46;
const SALES_TAX = 7034.18;
const PRE_TAX = CONTRACT_VALUE - SALES_TAX;
const PRODUCT_COST = 64342.41;
const LABOR_BILLED = 24762.50;
const SCHEDULE_A_LABOR = 21987.50;
const PRODUCT_MARKUP = 20920.37;
const TOTAL_HOURS = 169;
const SCHEDULE_A_PROFIT = 30717.55;
const SW_SCHEDULE_A = SCHEDULE_A_PROFIT * 0.49;

const waterfallData = [
  { name: "Contract Value", value: CONTRACT_VALUE, fill: "#1e3a5f" },
  { name: "Sales Tax", value: -SALES_TAX, fill: "#94a3b8" },
  { name: "Pre-Tax Revenue", value: PRE_TAX, fill: "#1e40af" },
  { name: "Product Cost", value: -PRODUCT_COST, fill: "#dc2626" },
  { name: "Labor Billed", value: -LABOR_BILLED, fill: "#f59e0b" },
  { name: "Product Markup", value: PRODUCT_MARKUP, fill: "#16a34a" },
  { name: "Schedule A Profit", value: SCHEDULE_A_PROFIT, fill: "#059669" },
];
...
const findings = [
  { id: 1, title: "Product markups ARE properly captured", color: "border-green-500 bg-green-50", icon: "✅", detail: "Product dealer costs of $64,342.41 match between D-Tools and Schedule A. Markup revenue of $20,920.37 flows into the Profit Pool." },
  { id: 2, title: "Labor profit is buried inside \"costs\"", color: "border-red-500 bg-red-50", icon: "🔴", detail: "Schedule A lists Labor Cost as $21,987.50 — but this is billed to the customer, not TCL's actual labor expense. Industry rates suggest ~$5,915 true cost, meaning ~$18,800 in labor profit never enters the Profit Pool." },
  { id: 3, title: "$2,775 labor discrepancy", color: "border-yellow-500 bg-yellow-50", icon: "⚠️", detail: "Schedule A states $21,987.50 in labor costs. D-Tools shows $24,762.50 billed. The $2,775 gap is unexplained." },
  { id: 4, title: "Negative-margin items reduce shared profit pool", color: "border-yellow-500 bg-yellow-50", icon: "⚠️", detail: "7 product categories totaling $448.68 are sold below dealer cost. These losses compress the Profit Pool that Streamwalkers participates in." },
  { id: 5, title: "Sales tax dilutes the margin percentage", color: "border-yellow-500 bg-yellow-50", icon: "⚠️", detail: "The $117,059.46 Contract Value includes ~$7,034 in sales tax. Including it reduces apparent margin from 27.93% to 26.26%." },
];
...
function SensitivitySlider() {
  ...
  const r = rate[0];
  const trueCost = TOTAL_HOURS * r;
  const hiddenProfit = SCHEDULE_A_LABOR - trueCost;
  const trueProfit = PRE_TAX - PRODUCT_COST - trueCost;
  const trueMargin = ((trueProfit / PRE_TAX) * 100).toFixed(2);
  const swShouldBe = trueProfit * 0.49;
  const shortfall = swShouldBe - SW_SCHEDULE_A;

  const lineData = Array.from({ length: 10 }, (_, i) => {
    const hr = 20 + i * 5;
    const tp = PRE_TAX - PRODUCT_COST - TOTAL_HOURS * hr;
    return { rate: `$${hr}/hr`, sw49: Math.round(tp * 0.49), baseline: Math.round(SW_SCHEDULE_A) };
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label="True Labor Cost" value={`$${trueCost.toLocaleString()}`} sub={`${TOTAL_HOURS} hrs × $${r}`} />
        ...
        <MetricCard label="SW 49% Should Be" value={`$${Math.round(swShouldBe).toLocaleString()}`} sub={shortfall > 0 ? `+$${Math.round(shortfall).toLocaleString()} vs Schedule A` : ""} accent="text-[#C42020]" />
      </div>
      <TCLCard className="p-4">
        <h4 className="text-sm font-semibold text-gray-700 mb-3">SW 49% Share vs Assumed Labor Rate</h4>
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={lineData}>
            ...
            <Line type="monotone" dataKey="sw49" stroke="#C42020" strokeWidth={2} name="SW 49% (True)" dot={false} />
            <Line type="monotone" dataKey="baseline" stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" name="Schedule A Baseline" dot={false} />
            <ReferenceLine y={Math.round(SW_SCHEDULE_A)} stroke="#94a3b8" strokeDasharray="3 3" />
          </LineChart>
        </ResponsiveContainer>
      </TCLCard>
      <TCLCard className="p-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="font-bold"><td className="py-2">Total</td><td className="py-2">{TOTAL_HOURS}</td><td></td><td className="py-2 text-right">${LABOR_BILLED.toLocaleString()}</td></tr>
          </tbody>
        </table>
      </TCLCard>
    </div>
...
export default function ProfitAnalysisView({ onBack }: { onBack?: () => void }) {
  return (
    <div>
      ...
      <div className="p-7 space-y-8 max-w-[1200px]">
        {/* Header */}
        <div>
          <h2 className="text-xl font-bold text-gray-900">TCL Profit Analysis & Amendment 1-A</h2>
          <p className="text-sm text-gray-500 mt-1">Wall Residence (P-1) — Revenue Waterfall & Margin Audit</p>
        </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <MetricCard label="Schedule A Margin" value="26.26%" sub="On gross contract value" />
            ...
            <MetricCard label="Labor Data Gap" value="$2,775" sub="Schedule A vs D-Tools" accent="text-yellow-600" />
          </div>
        ...
        <section>
          ...
          <TCLCard className="mt-3 p-4">
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  { layer: "Contract Value", amount: CONTRACT_VALUE },
                  { layer: "Sales Tax (pass-through)", amount: -SALES_TAX },
                  { layer: "Pre-Tax Revenue", amount: PRE_TAX },
                  { layer: "Product Dealer Cost", amount: -PRODUCT_COST },
                  { layer: "Labor Billed to Customer", amount: -LABOR_BILLED },
                  { layer: "Product Markup Revenue", amount: PRODUCT_MARKUP },
                  { layer: "Schedule A Stated Profit", amount: SCHEDULE_A_PROFIT },
                ].map((r) => (
                  <tr key={r.layer} className="border-b border-gray-100">
                    ...
                    <td className="py-2 text-right text-gray-500">{((Math.abs(r.amount) / CONTRACT_VALUE) * 100).toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section>
          ...
          <TCLCard className="p-4">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={belowCostItems} layout="vertical" margin={{ left: 180 }}>
                <CartesianGrid strokeDasharray="3 3" />
          ...
          <TCLCard className="mt-3 p-4">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="font-bold"><td className="py-2">Total Impact</td><td></td><td></td><td></td><td className="py-2 text-right text-red-600">-$448.68</td></tr>
              </tbody>
            </table>
            <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
              <strong>Impact:</strong> These below-cost items reduce the shared Profit Pool by $448.68, resulting in ~$219.85 reduced distribution to Streamwalkers (49%).
            </div>
          </TCLCard>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-3">
            <MetricCard label="Margin on Gross" value="26.26%" sub="Including sales tax" />
            <MetricCard label="Margin on Pre-Tax" value="27.93%" sub="Excluding sales tax" accent="text-green-600" />
            <MetricCard label="Dilution Effect" value="-1.67%" sub="Tax pass-through impact" accent="text-yellow-600" />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            {findings.map((f) => (
              <TCLCard key={f.id} className={`p-4 border-l-4 ${f.color}`}>
                <div className="flex items-start gap-3">
                  <span className="text-lg">{f.icon}</span>
                  <div>
                    <div className="text-sm font-bold text-gray-800">Finding {f.id}: {f.title}</div>
